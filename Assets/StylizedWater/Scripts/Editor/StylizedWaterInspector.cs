...
namespace StylizedWaterShader
{
    ...
    public class StylizedWaterInspector : Editor
    {
        ...
        void DoHeader()
        {
            ...
            EditorGUILayout.HelpBox("Unity 6+ is outside of the supported and compatible scope for this asset", MessageType.Error);
            #endif
            
            if (GraphicsSettings.currentRenderPipeline)
            {
                var newAssetVersion = 2;
                #if UNITY_6000_0_OR_NEWER
                newAssetVersion = 3;
                #endif
                
                EditorGUILayout.HelpBox($"The Universal Render Pipeline is in use,\nwhich this asset does not support.\n\nSee the Stylized Water {newAssetVersion} asset...", MessageType.Error);
                ...
                using (new EditorGUILayout.HorizontalScope())
                {
                    ...
                    if (GUILayout.Button("Open Asset Store", GUILayout.Width(120)))
                    {
                        #if UNITY_6000_0_OR_NEWER
                        //SW3
                        Application.OpenURL("https://assetstore.unity.com/packages/vfx/shaders/287769?aid=1011l7Uk8&pubref=sw1editor");
                        #else
                        //SW2
